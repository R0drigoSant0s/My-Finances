...
import { Menu, Home, CreditCard, BarChart2, LineChart, Settings, LogOut, X, Moon, Sun } from 'lucide-react';
...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  const isDashboardActive = location === '/home' || location === '/';

  // Renderiza o menu
  const renderMenu = () => (
    <nav className="p-3 pt-2 flex flex-col h-full">
      <div className="flex-grow">
        <ul className="space-y-3">
          {menuItems.map((item) => {
            // Verificar se é o Dashboard e se estamos na home ou na raiz
      ...
      <div className="space-y-3 mt-5">
        ...
        <button 
          onClick={toggleTheme}
          className="w-full flex items-center px-3 py-2.5 rounded-md text-sm font-medium transition-colors text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800"
        >
          {theme === 'dark' ? (
            <>
              <Sun className="w-5 h-5 mr-3 text-amber-500 dark:text-amber-400" />
              Modo claro
            </>
          ) : (
            <>
              <Moon className="w-5 h-5 mr-3 text-indigo-600 dark:text-indigo-400" />
              Modo escuro
            </>
          )}
        </button>
      ...
      {isMobile ? (
        ...
        <div className="flex flex-col h-screen overflow-hidden bg-slate-100 dark:bg-slate-900">
          ...
          <div className={`${sidebarClass} bg-white dark:bg-gray-900 h-full overflow-y-auto safe-area-padding`}>
            ...
            <div className="mt-1 flex flex-col h-[calc(100%-60px)]">
              {renderMenu()}
            </div>
          </div>
